...
describe("assembleAwareness", () => {
  ...
  it("skips underscore extensions, dependencies, git, and build output from the tree", async () => {
    await makeAgent(tmp, { "purpose.md": "p" });
    for (const name of ["_assets", "_scratch", "node_modules", ".git", "dist", "build"]) {
    ...
    expect(block).not.toContain("_scratch/");
