...
- **The base extension rule is negative.** Exact `_agent` is core; the first other underscore-prefixed directory owns an opaque subtree. Generic readers stop there without inventory, discovery, warnings, or named-extension knowledge. Classification reports role only—Git state and explicit selected paths decide mutation.
- **Document semantics, not machinery.** A named extension states its exact name and placement, payload, authored references, unaware behavior, portable operations and vectors, and extension-owned format evolution. A readable contract is expected for documented extensions and shared vectors for standard ones; schemas, examples, skills, and adapters remain optional. Do not infer a registry, manifest, package system, loader, or runtime support API.
- **Supporting payload is not an asset subsystem.** `_assets/` is one optional standard extension with a portable lexical reference rule. Filesystem reads, Markdown parsing, rendering, MIME/size limits, upload, access, and storage remain consumer concerns.
