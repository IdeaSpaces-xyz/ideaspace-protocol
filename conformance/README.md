...
lib here, or another language/runtime) actually conforms. Repository-shape, root-identity, and local-effect conformance are distinct claims: absence remains valid for an ordinary reader; an identity implementation executes the root-identity vectors; and an effect implementation executes every local-effect manifest coverage tag.
