...
- **`validateSpace(root)`** — shipped from the reference library
  ...
  - **warn** — drift signals: a missing `foundation.md`, named-but-absent
    contract files (`guide.md`/`purpose.md`/`now.md`), and skipped unknown underscore-prefixed
    infrastructure folders. Recognized `_assets/` payload is skipped silently. Drift never fails
    conformance.
...
lib here, or another language/runtime) actually conforms. Repository-shape, assets, root-identity,
and local-effect conformance are distinct claims: an assets implementation executes the assets
vectors; identity absence remains valid for an ordinary reader; an identity implementation executes
the root-identity vectors; and an effect implementation executes every local-effect manifest
coverage tag.
