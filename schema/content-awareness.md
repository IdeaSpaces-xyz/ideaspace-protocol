...
| `skills` | `_agent/skills/*.md` composed along the root→position stack — the union across levels, a deeper same-named skill shadowing its ancestor's — with names, source paths, composing levels, and summaries. A skill's `description` frontmatter (its trigger condition) is the summary source, falling back to `summary`, then first body line. `README.md` is the folder's surface and never appears in the roster. |
